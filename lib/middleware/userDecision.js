...
module.exports = function userDescision(server, options, parse, issue) {
  ...
  return function userDescision(req, res, next) {
    ...
    parse(req, function(err, ares, info) {
      ...
      req.oauth.res = ares || {};
      if (info) {
        utils.merge(req.oauth.info, info);
      }
      ...
      if (allow) {
        ...
        if (arity == 6) {
          issue(req.oauth.authz.token, req.oauth.user, req.oauth.res, req.oauth.req, req.oauth.info, issued);
        } else if (arity == 5) {
          issue(req.oauth.authz.token, req.oauth.user, req.oauth.res, req.oauth.req, issued);
        } else if (arity == 4) {
