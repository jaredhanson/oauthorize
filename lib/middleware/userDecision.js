    ...
    parse(req, function(err, ares) {
      ...
      if (allow) {
        issue(req.oauth.authz.token, req.oauth.user, req.oauth.res, function(err, verifier) {
          ...
          // TODO: If no callbackURL available, next to let app render the verifier.
        ...
        //
        // This middleware simply `next`'s in this case, and it is expected that
        // the application will implement middleware to handle this situation as
        // desired, by checking if `req.oauth.res.allow` is `false`.
        
        // TODO: Implement default behavior, redirecting with oauth_problem, along
