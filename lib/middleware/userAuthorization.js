 ...
 * @api public
 */
module.exports = function userAuthorization(server, options, parse, validate, immediate) {
  ...
  if (!validate) {
    // fn signature: userAuthorization(server, options, validate)
    validate = parse;
  ...
  return function userAuthorization(req, res, next) {
    ...
    function validated(err, consumer, callbackURL, areq, locals) {
      ...
      req.oauth.req = areq || {};
      req.oauth.user = req.user;
      
      function immediated(err, allow, info, locals) {
        if (err) { return next(err); }
        
        // TODO: Handle allow = true and serialize remaining arguments correctly
      ...
      if (arity == 4) {
        immediate(req.oauth.consumer, req.oauth.user, req.oauth.req, immediated);
      } else if (arity == 5) {
        immediate(req.oauth.consumer, req.oauth.user, req.oauth.req, locals, immediated);
      } else { // arity == 3
        immediate(req.oauth.consumer, req.oauth.user, immediated);
      }
    }
